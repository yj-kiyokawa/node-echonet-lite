...
*   - type    | String | Required | Network type code, 'lan'.
...
var EchonetLite = function(params) {
	...
	if(('type' in params) && typeof(params['type'] === 'string') && params['type'] !== '') {
		...
		if(type.match(/^(lan)$/)) {
			...
			throw new Error('The value of the "type" property must be "lan".');
		}
	}
	...
	this.timeout = 200;
	if(('timeout' in params) && typeof(params['type'] === 'number')) {
	...
	this.mELNet.setDataCallback((buf, device) => {
		...
		for (const key in this.callbacks) {
			if (this.callbacks.hasOwnProperty(key)) {
				...
				var now = Date.now();
				if (element && element.createAt && element.createAt + this.timeout < now) {
					if(element['once'] === true) {
						//	callback with timeout error
						element['callback'](new Error("timeout"), null);
						//	delete callback
						this._delCallback(key);
					}
				}
			}
		}
